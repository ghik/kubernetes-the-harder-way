\[ [macOS/ARM64](../../macos/docs/05_Installing_Kubernetes_Control_Plane.md) | **Linux/AMD64** \]
...
```
91bdf612a6839630, started, control0, https://192.168.42.11:2380, https://192.168.42.11:2379, false
bb39bdb8c49d4b1b, started, control2, https://192.168.42.13:2380, https://192.168.42.13:2379, false
...
to resolve to a mysterious, unassigned address 192.168.42.21. This is a virtual IP, and it is now time to properly
set it up.
...
First, we need to make sure all the control nodes recognize the virtual IP 192.168.42.21 as their own.
...
```bash
sudo ip addr add 192.168.42.21/32 dev enp0s1
```
...
```bash
sudo ip addr add 192.168.42.21/32 dev lo
```
...
```bash
$ ping 192.168.42.21
PING 192.168.42.21 (192.168.42.21): 56 data bytes
...
```bash
sudo ip addr add 192.168.42.21/32 dev <interface-name>
```
...
```bash
sudo ipvsadm -A -t 192.168.42.21:6443 -s rr
sudo ipvsadm -a -t 192.168.42.21:6443 -r 192.168.42.11:6443 -g
sudo ipvsadm -a -t 192.168.42.21:6443 -r 192.168.42.12:6443 -g
sudo ipvsadm -a -t 192.168.42.21:6443 -r 192.168.42.13:6443 -g
```
...
virtual=192.168.42.21:6443
    servicename=kubernetes
    real=192.168.42.11:6443 gate
    real=192.168.42.12:6443 gate
    real=192.168.42.13:6443 gate
    scheduler=wrr
    checktype=negotiate
    service=https
    request="healthz"
