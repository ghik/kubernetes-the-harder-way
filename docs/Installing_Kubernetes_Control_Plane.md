...
- [Installing Kubernetes Control Plane](#installing-kubernetes-control-plane)
...
Make sure you have completed all the previous chapters, your VMs are [running](Launching_the_VM_Cluster.md#a-script-to-launch-them-all) 
and have all the certificates, keys and kubeconfigs [deployed](Bootstrapping_Kubernetes_Security.md#distributing-certificates-and-keys).
...
`tmux` with pane synchronization, as [described](Launching_the_VM_Cluster.md#tmux-crash-course) in one of the 
...
of the security assumptions from the [previous chapter](Bootstrapping_Kubernetes_Security.md). The other ones simply
...
* the security related options (certs, etc.) simply reflect the assumptions made in the
  [previous chapter](Bootstrapping_Kubernetes_Security.md).
...
[we have set up earlier](Preparing_Environment_for_a_VM_Cluster.md#impromptu-bash-templating-for-cloud-init-files)
