...
We choose **192.168.42.0/24** as the CIDR for the network, where 192.168.42.1 is the host machine address.
Let's make this a reality:

```bash
sudo ip addr add 192.168.42.1/24 dev kubr0
```
...
3: kubr0: <NO-CARRIER,BROADCAST,MULTICAST,UP> mtu 1500 qdisc noqueue state DOWN group default qlen 1000
    ...
    inet 192.168.42.1/24 scope global kubr0
...
Packets leaving the 192.168.42.0/24 network must have their source addresses translated in order for the returning
packets to be routable from the external world. On Linux, this is typically done by setting up NAT in `iptables`:

```bash
sudo iptables -t nat -A POSTROUTING ! -o kubr0 -s 192.168.42.0/24 -j MASQUERADE
```

The condition `! -o kubr0 -s 192.168.42.0/24` ensures that NAT is performed for packets originating from
192.168.42.0/24 and destined for outgoing interface other than `kubr0`.
...
2: enp0s2: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc fq_codel state UP group default qlen 1000
    ...
    inet 192.168.42.10/24 metric 100 brd 192.168.42.255 scope global dynamic enp0s2
...
default via 192.168.42.1 dev enp0s2 proto dhcp src 192.168.42.10 metric 100
192.168.42.0/24 dev enp0s2 proto kernel scope link src 192.168.42.10 metric 100
192.168.42.1 dev enp0s2 proto dhcp scope link src 192.168.42.10 metric 100
```
...
worker0: 192.168.42.14                          -- link: enp0s2
         (worker0.kubenet)
```
...
This will generate a keypair: a private key (`~/.ssh/id_ed25519`) and a public key (`~/.ssh/id_ed25519.pub`).
